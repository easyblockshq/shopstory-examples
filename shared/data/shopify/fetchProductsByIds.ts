...
export const fetchProductsByIds = async (ids: string[], options: { includeRelated: boolean } = { includeRelated: false }): Promise<ShopifyProduct[]> => {
  const data: any = await fetchShopify(fetchProductsByIdsQuery, {
    ids
  })

  const products : ShopifyProduct[] = data.nodes.map(mapProduct).filter((product: any) => !!product)
